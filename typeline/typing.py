...
def shrink_types(types):
    ...
    # relationships)
    union = Union[types]
    if not safe_issubclass(union, _Union):
        return union

    return Union[tuple(sorted(list(union.__args__), key=str))]
...
def safe_issubclass(t, base_class):
    try:
        return issubclass(t, base_class)
    except TypeError:
        return False
