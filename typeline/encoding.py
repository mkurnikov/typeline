...
from typeline.db.base import CallTraceThunk
from typeline.exceptions import InvalidTypeError
from typeline.tracing import CallTrace, ClassPropsTrace
from typeline.typing import NoneType
from typeline.util import (
    get_func_in_module,
    get_name_in_module,
)
...
def type_to_dict(typ: type) -> TypeDict:
    ...
    # if hasattr(typ, '_gorg') and typ._gorg == Callable:
    #     print(11)
...
def type_from_json(typ_json: str) -> type:
    ...
    if isinstance(type_dict, str):
        type_dict = json.loads(type_dict)
...
class ClassPropsTraceRow(object):
    def __init__(self, module: str, qualname: str, props: Dict[str, Any]):
        self.module = module
        self.qualname = qualname
        self.props = props

    def to_trace(self) -> ClassPropsTrace:
        return ClassPropsTrace(module=self.module,
                               qualname=self.qualname,
                               class_props={name: type_from_dict(type_dict) for name, type_dict in self.props.items()})
...
class CallTraceRow(CallTraceThunk):
    ...
    def to_trace(self) -> CallTrace:
        ...
        # obj_dict = None
        # if self.self_params:
        #     obj_dict = arg_types_from_json(self.self_params)
